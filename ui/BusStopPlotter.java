...
public class BusStopPlotter extends MapViewOverlay {
    ...
    public void markStops(Location currentLocation) {
        System.out.println("MARK STOPS GETTING CALLED");
    ...
    public void updateMarkerOfNearest(Stop nearest) {
        ...
        if (nearest != null) {
            Marker marker;
            if (stopMarkerMap.containsKey(nearest)) {
                marker = getMarker(nearest);
            } else {
            ...
            if (nearestStnMarker != null && !(marker == nearestStnMarker))
